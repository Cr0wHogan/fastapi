...
def get_user_by_id_virtual_world(db: Session, id_virtual_world: str):
    return db.query(models.User).filter(models.User.id_virtual_world == id_virtual_world).first()
...
def create_user(db: Session, user: schemas.UserCreate):
    ...
    db_user = models.User(id_virtual_world=user.id_virtual_world,nombre=user.nombre)#, hashed_password=fake_hashed_password)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
